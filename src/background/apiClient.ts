...
/**
 * Error thrown when parsed translation count does not match requested text count
 */
export class ParseCountMismatchError extends Error {
  constructor(
    ...
    super(
      message ||
        `Translation count mismatch: expected ${expectedCount}, got ${actualCount}`,
    );
    this.name = "ParseCountMismatchError";
  }
}
...
export class OpenRouterClient {
  ...
  async translate(texts: string[], targetLanguage: string): Promise<string[]> {
    ...
    if (!this.config?.model || this.config.model.trim() === "") {
      ...
      throw new Error(
        "Model not configured or empty. Please specify a model in the settings.",
      );
    }
...
${combined}`;
  ...
  private splitWithoutSeparator(
    ...
    if (
      ...
      Math.abs(byParagraph.length - expectedCount) <
        Math.abs(byLine.length - expectedCount)
    ) {
      logger.warn(
    ...
    if (byLine.length > 1) {
      logger.warn(
        `Using line split (${byLine.length}) as fallback, expected ${expectedCount}`,
      );
      return byLine;
    }
  ...
  private parseResponse(content: string, expectedCount: number): string[] {
    ...
    if (cleanedContent.includes(this.TEXT_SEPARATOR)) {
      parsedTranslations = cleanedContent
      ...
      parsedTranslations = this.splitWithoutSeparator(
        cleanedContent,
        expectedCount,
      );
    }
    ...
    if (parsedTranslations.length !== expectedCount) {
      logger.warn(
        `Translation count mismatch after parsing: expected ${expectedCount}, got ${parsedTranslations.length}`,
      );
      throw new ParseCountMismatchError(
        expectedCount,
