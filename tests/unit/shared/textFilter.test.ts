...
describe('textFilter', () => {
  describe('shouldTranslateText', () => {
    ...
    it('should return false for numbers only', () => {
      ...
      expect(shouldTranslateText('+1 (555) 123-4567').shouldTranslate).toBe(false);
    });
    ...
    it('should return false for URLs', () => {
      expect(shouldTranslateText('https://example.com').shouldTranslate).toBe(false);
      expect(shouldTranslateText('http://example.com/path').shouldTranslate).toBe(false);
      expect(shouldTranslateText('www.example.com').shouldTranslate).toBe(false);
    });

    it('should return false for email addresses', () => {
      expect(shouldTranslateText('user@example.com').shouldTranslate).toBe(false);
      expect(shouldTranslateText('test.user+tag@domain.co.jp').shouldTranslate).toBe(false);
    });
  ...
  describe('reconstructResults', () => {
    it('should reconstruct results with translations and skipped texts', () => {
      ...
      const results = reconstructResults(translations, originalIndices, skippedIndices, 5);
    ...
    it('should handle no skipped texts', () => {
      ...
      const results = reconstructResults(translations, originalIndices, skippedIndices, 2);
