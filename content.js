(() => {
	...
	function showSelectionIcon(e) {
		...
		if (selection && selection.toString().trim().length > 1) {
			// En az 2 karakter seçilmiş olsun
	...
	document.addEventListener(
		...
		() => {
			if (
				selectionIcon &&
				selectionIcon.style.display === "block" &&
				window.getSelection().toString().trim().length > 1
			) {
    ...
    `;
		...
		const batches = createBatches(textNodes, 1600); // Reduced to 1600 characters per batch for better reliability
	...
	async function translateSpecialElements(targetLanguage) {
		...
		for (const element of specialElements) {
			...
			if (currentLength + textLength > 1600 && currentBatch.length > 0) {
	...
	async function translateNodes(nodes, targetLanguage) {
		...
		const batches = createBatches(nodes, 1600);
